...
sequenceDiagram
    ...
    Note right of browser: Browser captures the note content

    browser->>server: POST https://studies.cs.helsinki.fi/exampleapp/new_note
    activate server
    Note right of server: Server processes the new note and stores it
    server-->>browser: Redirect response (e.g., 302 to /notes)
    deactivate server
    ...
    activate server
    server-->>browser: [{ "content": "HTML is easy", "date": "2023-1-1" }, { "content": "New note content", "date": "2024-6-19" }, ... ]
    deactivate server
